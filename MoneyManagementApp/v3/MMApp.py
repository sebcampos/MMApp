...
class DB():
    ...
    def delete(self, table, value):
        df = self.fetch(table)
        df.drop(df.loc[df[df.columns[0]] == value].index, inplace=True)
    ...
    def replace(self, new_data, table, index):
        df = self.fetch(table)
        df.loc[df[df.columns[0]] == index] = new_data.values  
...
class ViewTransactionScreen(Screen):
    ...
    def delete_transaction(self, app, transaction_num):
        ...
        try:
            app.db.delete("Scheduled", int(transaction_num))
...
class AddTransactionScreen(Screen):
    ...
    def schedule(self, date, data=[]):
        ...
        if len(data) != 0:
            ...
            transaction_id = data.values[0][0]
    ...
    def update_db(self, app):
        ...
        try:
            if self.transaction_exists == True:
                app.db.replace(pandas.DataFrame({
                ...
                transaction_id = self.new_id
                cb = BubbleButton(text="Transaction Updated\n\n\n\n   Close")
                pu = Popup(title="Success", content=cb, size_hint=(.5, .5))
                cb.bind(on_press=pu.dismiss)
                pu.open()
...
class BudgetScreen(Screen):
    def load_budgets(self, app):
        budgets = app.db.fetch("Budgets")
        print(budgets)

class WalletsScreen(Screen):
    def load_wallets(self, app):
        wallets = app.db.fetch("Wallets")
        self.wallets = wallets
        gl = self.ids["gl"]
        for row in wallets.values:
            gl.add_widget(BubbleButton(text=str(i)))
            for item in row:
                gl.add_widget(Label(text=str(item)))
...
class GoalsScreen(Screen):
    def load_goals(self, app):
        goals = app.db.fetch("Goals")
        gl = self.ids["gl"]
